...
def main():
        ...
        for (y, x), rows in cache.items():
            ...
            if not os.path.isfile(path_to_outfile):
                with gzip.open(path_to_outfile, "wt") as _:
                    ...
                    writer.writerow(
                        ["iso-date", "year", "month", "day", "doy", "tmin", "tavg", "tmax", "precip", "relhumid",
                         "globrad", "wind", "airpress", "airPressureAtSeaLevel", "specificHumidity", "snowfallFlux",
                         "surfaceDownwellingLongwaveRadiation"])
                    writer.writerow(
                        ["[]", "[]", "[]", "[]", "[]", "[°C]", "[°C]", "[°C]", "[mm]", "[%]", "[MJ m-2]", "[m s-1]",
                         "[hPa]", "[hPa]", "[g kg-1]", "[mm]", "[MJ m-2]"])
    ...
    for start_year, end_year in sorted(files.keys()):
        ...
        for year in range(start_year, end_year + 1):
            ...
            for doy_i in range(int(config["start_doy"]) - 1,
                               min(int(config["start_doy"]) + int(config["end_plus_doys"]), days_in_year),
                               days_per_loop):
                ...
                for y in range(int(config["start_y"]) - 1,
                               ref_data.shape[1] if not config["end_y"] else int(config["end_y"])):

                    # for x in range(ref_data.shape[2]):
                    for x in range(int(config["start_x"]) - 1,
                                   ref_data.shape[2] if not config["end_x"] else int(config["end_x"])):
                        ...
                        for i in range(no_of_days):
                            ...
                            row = [
                                ...
                                str(round(data["precip"][i, y, x] * secs_per_day, 2)),
                                # kg m-2 s-1 -> [mm] = 100mm*100mm*100mm (1L=1kg) / 1000mm*1000mm (1m*1m) = 1
                                str(round(data["relhumid"][i, y, x], 2)),  # [%]
                                str(round(data["globrad"][i, y, x] * W_per_m2_per_s_to_MJ_per_m2_per_day, 4)),
                                # W m-2 s-1 -> [MJ m-2] -> 1 W s = 1 J
                                str(round(data["wind"][i, y, x], 2)),  # [m s-1]
                                str(round(data["airpress"][i, y, x] / 100.0, 2)),  # [hPa]
                                str(round(data["airPressureAtSeaLevel"][i, y, x] / 100.0, 2)),  # [hPa]
                                str(round(data["specificHumidity"][i, y, x] * 1000.0, 2)),  # kg kg-1 -> [g kg-1]
                                str(round(data["snowfallFlux"][i, y, x] * secs_per_day, 2)),
                                # kg m-2 s-1 -> [mm] = 100mm*100mm*100mm (1L=1kg) / 1000mm*1000mm (1m*1m) = 1
                                str(round(data["surfaceDownwellingLongwaveRadiation"][
                                              i, y, x] * W_per_m2_per_s_to_MJ_per_m2_per_day, 4)),
                                # W m-2 s-1 -> [MJ m-2] 1 W s = 1 J
                                # str(round(vp, 2))
                            ]
                ...
                print("running year", year, "(doy currently:", doy_i, ") took", (clock_end_year - clock_start_year),
                      "seconds so far")
    ...
    copy_to_path = config["path_to_output"]
